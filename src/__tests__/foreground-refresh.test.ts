...
describe('ADR-007 item 3 — a rotated session that cannot be stored is surfaced', () => {
  ...
  it('clears the failure when a later write succeeds', async () => {
    const harness = createHarness();
    harness.refuseWrites = true;
    await expect(harness.storage.setItem(SESSION_KEY, sessionJson('refresh-v2'))).rejects.toThrow();
    ...
    // The flag means "the most recent write failed", not "a write failed once".
    expect(peekSessionPersistenceFailure()).toBeNull();
    expect(
