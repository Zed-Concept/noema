 ...
 *
 * RE-BASED INTO 006c FOR FIX CYCLE 2 (one fake enrichment; the schedules,
 * assertions, and pinned trees are unchanged): REVIEW-024 finding 1 makes
 * the shipped consult corroborate ABSENCE with the parent directory's
 * LISTING, an expo-file-system surface the 006b fake did not model — under
 * the old fake a missing `list()` reads as a refused listing, and the fixed
 * consult correctly treats that as outstanding, which turns the fresh-mount
 * and Known-limit schedules spuriously red at the fix-2 head. The fake's
 * `Paths.document` now lists the same persistent fake medium the File
 * reads. At the pinned RED tree the old consult never calls `list()`, so
 * the enrichment is inert there and the RED control is undisturbed.
 * `../006b-session-durability-fix1/review023-probe.tsx` stays
 * byte-identical (immutable record); THIS copy is the cycle-2 instrument.
 */
...
jest.mock('expo-file-system', () => {
  ...
  return {
    ...
    Paths: {
      document: {
        // The parent-directory listing the fixed consult corroborates
        // absence with (REVIEW-024 finding 1). Reads the same persistent
        // fake medium the File does.
