#!/usr/bin/env bash
#
# Mutation harness for 006c — session durability fix cycle 2 (Unit E, CTRL-006).
#
# The harness mechanism is inherited from ../006b-session-durability-fix1/
# mutants.sh unchanged; the battery is this cycle's own. It re-bases the 006b
# mutants that still apply to the fixed code (M1–M21, M23–M25 — anchors moved
# by the fix are updated; M22 is REBUILT because REVIEW-024 finding 1 deleted
# the branch the old M22 restored half of: the new M22 restores the exact
# REVIEW-024 defect, the exists-corroboration after a thrown read), and adds
# mutants for every behaviour this cycle introduced: the publication barrier
# and each of its halves (M26–M28), the refusal-resolves-to-signedOut rule
# (M31), the synchronous flag order (M29), the take-to-cache single act
# (M30), and the two finding-1 absence-observation guards (M32–M33).
#
...
cd "$(dirname "$0")/../../../.." || exit 1
OUT="${1:-docs/05-quality/evidence/006c-session-durability-fix2}"
...
{
  echo "# Mutation sensitivity — every 006c claim with an instrument, checked both ways."
  echo "#"
