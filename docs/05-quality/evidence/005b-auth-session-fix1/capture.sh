#!/usr/bin/env bash
#
# Evidence producer for 005b — auth and session v1, fix cycle 1 (Unit D, CTRL-005).
#
# Writes every artifact in this directory except README.md, mutants.txt, and
# stability.txt. Fails closed: a scan whose positive control does not match, a
...
# service, and no Supabase endpoint is touched at all. The Expo CLI loads a
...
# showed was never measured.
{
  ...
  adapter_status=0
  for suite in secure-store-adapter supabase-client session-storage-platform secure-store-accessibility; do
...
# ------------------------------------------------ session/OTP property transcript
{
  ...
  echo "# device-local sign-out, and the AppState gate on token auto-refresh."
  ...
  npm test -- --ci --runInBand --verbose --testPathPattern 'auth-provider' 2>&1 | mask
  code=${PIPESTATUS[0]}
  echo "exit: $code"; [ "$code" -eq 0 ] || note_failure
