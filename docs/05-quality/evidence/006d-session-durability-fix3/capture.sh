#!/usr/bin/env bash
#
# Evidence producer for 006d — session durability fix cycle 3 (Unit E, CTRL-006).
#
# Inherited from ../006c-session-durability-fix2/capture.sh. Cycle 3 is
# SUBTRACTION ONLY (ruling 28): no check, scan, exclusion, or gate in this
# script changed from 006c — only this prose and the prose written into the
# artifacts, which 006c stated as universals and REVIEW-025 finding 2
# disproved by direct counterfactual.
#
#   WHAT THE GATED ARTIFACTS ACTUALLY DEPEND ON — stated honestly, no
#   "by construction". Three inputs, not one: (1) the bound product-tree
#   OIDs binding.txt records (`git rev-parse HEAD:<path>` for src, the
#   manifests, app.json, and the config files the gates read); (2)
#   TypeScript anywhere tsconfig.json includes — `**/*.ts(x)`, docs/
#   INCLUDED — because the typecheck gate reads it (REVIEW-025's docs-only
#   type-invalid .ts counterfactual changed gates.txt with every bound OID
#   equal); and (3) the red-lane range listing's own range, which includes
#   docs/04-reviews — a review record added on top of an unchanged product
#   tree grows that listing (REVIEW-025's docs/04-only counterfactual:
#   12/12 bound OIDs equal, red-lane.txt drifted, stability exited 1). No
#   universal docs-only-commit stability claim is made; what IS claimed is
#   in the 006d README — stability demonstrated at the heads it names.
#
#   THE RED-LANE RANGE LISTING EXCLUDES EXACTLY docs/05-quality/evidence
#   AND docs/01-state — the paths that grow during evidence and records
#   commits. Review records under docs/04-reviews REMAIN IN THE LISTING,
#   so it grows with every review record; recorded, expected behaviour
#   outside the stability claim. No exclusion is added this cycle — that
#   would be a scanner change, and cycle 3 changes no behaviour. The
#   database-layer path FILTER still runs over the FULL unexcluded range,
#   so a SQL-shaped path anywhere (docs included) still fails the capture.
#
...
# known-issue-witness.txt, capture-refusal-control.txt, and ci.txt — NINE
# exceptions, counted here rather than asserted. known-issue-witness.txt is
# written by its own runner (an EXPECTED-RED witness of the REVIEW-025 Known
# Issues — see the README register — so it can never live inside a green
# capture). ci.txt is ABSENT BY DESIGN until after the push — a
...
# Every product path recorded in binding.txt. These trees are the FIRST of
# the gated artifacts' inputs, not the only one (the 006c "function of the
# product trees only" claim is withdrawn — ruling 28, REVIEW-025 finding 2):
# src is what typecheck/test/scan measure; the manifests are what deps.txt
# proves; app.json is what chrome.txt freezes; the config files steer
# typecheck, lint, test, and format; scripts is repo-tracked tooling;
# supabase and .github are the RED-lane frozen trees. docs/ is absent from
# THIS LIST yet still an input twice over: the tsconfig typechecks
# `**/*.ts(x)` including docs/ (gates.txt reads it), and the red-lane range
# listing includes docs/04-reviews (red-lane.txt grows with review records).
# Both stated in the header above and in the README; neither is bound here.
PRODUCT_PATHS=(
  src
  package.json
  package-lock.json
...
# ---------------------------------------------------------------- binding
# REVIEW-024 finding 3, narrowed by ruling 28: the artifacts record the
# product trees they measure — ONE of their inputs, with the two others
# (docs/ TypeScript under the typecheck gate; the red-lane listing's range)
# named in the header and the README. The run's own commit SHA goes in the
# non-gated binding-head.txt below; the base pin stays here because the
# range scans start from it. A run against a dirty tree still refuses
# BEFORE writing anything: artifacts describing a program that is not any
# commit are wrong in a way no individual artifact reveals.
...
BINDING_FAILED=0
{
  echo "# Binding — the bound product trees (REVIEW-024 finding 3, narrowed by"
  echo "# ruling 28 after REVIEW-025 finding 2). The product paths the gates"
  echo "# measure, recorded as git tree/blob object ids at run time, plus the"
  echo "# base pin the range scans start from — the one commit SHA in this"
  echo "# file, on the base: line below. These OIDs are one input to the gated"
  echo "# artifacts, not the only one: the typecheck gate reads TypeScript"
  echo "# under docs/ (tsconfig includes it), and red-lane.txt's range listing"
  echo "# includes docs/04-reviews. No universal docs-only-commit stability is"
  echo "# claimed; the README names the heads where stability was demonstrated."
  echo "# The run's own commit SHA and clean-tree verdict live in the"
  echo "# non-gated binding-head.txt."
  echo "#"
...
# ----------------------------------------------------------------- RED lane
{
  ...
  echo "# This artifact is a function of the RANGE, not of the bound trees"
  echo "# alone (ruling 28 / REVIEW-025 finding 2): its listing below excludes"
  echo "# exactly docs/05-quality/evidence and docs/01-state, so review"
  echo "# records under docs/04-reviews remain in it and it GROWS with every"
  echo "# review record — recorded, expected behaviour outside the stability"
  echo "# claim, which is bound to the heads the README names."
  echo
  ...
  echo "## Paths in the range outside the two excluded trees (docs/05-quality/"
  echo "## evidence and docs/01-state are excluded from THIS LISTING; everything"
  echo "## else in the range appears, review records under docs/04-reviews"
  echo "## INCLUDED — this listing grows with every review record. The"
  echo "## database-layer filter below still runs over the FULL unexcluded"
  echo "## range)"
  product_changed=$(git diff --name-only "$BASE" -- . ':(exclude)docs/05-quality/evidence' ':(exclude)docs/01-state'); prod_code=$?
  if git_exit "$prod_code" "git diff --name-only $BASE -- . (range listing)"; then
    echo "  paths in range outside the exclusions: $(printf '%s\n' "$product_changed" | grep -c . | tr -d ' ')"
