#!/usr/bin/env bash
#
# Runner for the REVIEW-024 findings 1+2 probe (Unit E fix cycle 3 re-run,
# CTRL-006). Carried into 006d from ../006c-session-durability-fix2/
# review024-probe.sh unchanged except for this directory's paths (ruling 28:
# cycle 3 changes no behaviour); review024-probe.tsx here is byte-identical
# to the 006c copy.
#
# Runs `review024-probe.tsx` against TWO committed trees, each in a disposable
# git worktree with this working copy's node_modules symlinked in:
#
#   1. the pinned REVIEWED CANDIDATE — 5f6d2e6c, the formal head REVIEW-024
#      measured, where the probe must be RED: its assertions encode the
#      reviewer's own schedules (the thrown-read-plus-lying-exists restart,
#      the fresh-sign-in-then-refused-refresh bootstrap, the mid-process
#      resolution re-read), and that head is where findings 1 and 2 live.
#      The RED run is this instrument's POSITIVE CONTROL (learning 14).
...
# `src/__tests__/review024-probe.test.tsx`, where the tree's own jest config
...
#
# The transcript is written to review024-probe.txt (or into the directory
...
OUT="${1:-$HERE}"
CANDIDATE=5f6d2e6ca873ff3b45d9d9a6e52d42bdebed30bd

mkdir -p "$OUT" 2>/dev/null
if [ ! -d "$OUT" ] || [ ! -w "$OUT" ]; then
  echo "review024-probe.sh: output directory '$OUT' is not writable — refusing to run." >&2
  exit 1
fi
...
if [ "$MB_STATUS" -ne 0 ] || [ "$MB" != "$CANDIDATE" ]; then
  echo "review024-probe.sh: CANDIDATE $CANDIDATE is not an ancestor of HEAD (git exit $MB_STATUS) — the pin is stale." >&2
  exit 1
fi

HEAD_SHA="$(git rev-parse HEAD)"; RP_STATUS=$?
if [ "$RP_STATUS" -ne 0 ] || [ -z "$HEAD_SHA" ]; then
  echo "review024-probe.sh: git rev-parse HEAD failed (exit $RP_STATUS) — refusing to run." >&2
  exit 1
fi
...
run_at() {
  ...
  cp "$HERE/review024-probe.tsx" "$wt/tree/src/__tests__/review024-probe.test.tsx"
  (cd "$wt/tree" && npx jest --ci --runInBand --testPathPattern review024-probe 2>&1) |
    sed -E \
...
{
  echo "# REVIEW-024 findings 1+2 probe — one probe, two trees."
  echo "# See review024-probe.tsx for what is asserted and review024-probe.sh for how."
  echo "#"
  echo "# candidate: $CANDIDATE (pinned; the formal head REVIEW-024 measured; must be RED)"
  ...
  if [ "$CAND_STATUS" -ne 0 ] && [ "$HEAD_STATUS" -eq 0 ]; then
    ...
    echo "the probe fails where REVIEW-024 findings 1 and 2 live and passes at"
...
} > "$OUT/review024-probe.txt"

if [ "$CAND_STATUS" -ne 0 ] && [ "$HEAD_STATUS" -eq 0 ]; then
  echo "review024-probe.sh: candidate RED, head GREEN — see review024-probe.txt."
  exit 0
fi
echo "review024-probe.sh: verdict FAILED (candidate exit $CAND_STATUS, head exit $HEAD_STATUS) — see review024-probe.txt." >&2
exit 1
