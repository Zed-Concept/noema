...
# when a local .env exists, which is machine state, not repo state).
{
  ...
  echo "# negative probe (REVIEW-009): .env.example must NOT be ignored. The path is"
  echo "# tracked, and default check-ignore never reports tracked paths — it exits 1"
  echo "# whatever the patterns say — so only --no-index evaluates the rules"
  echo "# themselves. Expected: exit 1, no output. If the !.env.example negation were"
  echo "# removed, .env* would match and this form would print the path and exit 0."
  echo "\$ git check-ignore --no-index .env.example"
  if git check-ignore --no-index .env.example; then example_ignored=0; else example_ignored=$?; fi
  echo "--- exit code: $example_ignored (1 = not ignored, decided by pattern evaluation) ---"
  echo "# same probe, verbose, to name the source of that decision: the line printed"
  echo "# must be .gitignore's !.env.example negation. In -v mode a negation match"
  echo "# counts as a match — exit 0 with the line printed is the expected result, and"
  echo "# the discriminating exit code stays with the plain form above. With the"
  echo "# negation removed, -v would name the .env* line instead."
  echo "\$ git check-ignore --no-index -v .env.example"
  if git check-ignore --no-index -v .env.example; then example_v=0; else example_v=$?; fi
  echo "--- exit code: $example_v (0 = a pattern matched; the line above names the deciding negation) ---"
