...
# artifact — gates.txt, deps.txt, secret-scan.txt — against the copy committed
# here, byte for byte. The exit status is the contract: 0 when every gated
# artifact matches in both runs, 1 otherwise. stability.txt itself is not
# gated — a gate cannot contain a run of itself (house precedent:
# ../002d-fix-loop-3/negative-control.txt).
#
...
gated=(gates.txt deps.txt secret-scan.txt)
fails=0
{
  ...
  for f in "${gated[@]}"; do
    for run in "$a" "$b"; do
      ...
      if cmp -s "$dir/$f" "$run/$f"; then
        printf '%-18s %s: identical\n' "$f" "$tag"
      else
        printf '%-18s %s: DIFFERS\n' "$f" "$tag"
