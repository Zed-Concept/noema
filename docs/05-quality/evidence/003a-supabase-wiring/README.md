...
|---|---|---|---|
| `gates.txt` | `capture.sh` | gated | normalization: all durations masked to `<duration>`; `npm ci` reduced to its added-package count (the audited clause and advisory/funding footers are registry-shaped); jest per-test/suite duration suffixes stripped; `env:` lines dropped (printed by the Expo CLI only when a local `.env` exists — machine state, not repo state). Fail-loudly covers all three missing-env cases (neither, URL-only, key-only); the ignore probes cover `.env`, `.env.local`, `.env.staging`, `.envrc`, `.envfoo` plus the negative probe that `.env.example` is not ignored and stays tracked |
...
| 5 | `.env.example` lists exactly the two variables, blank; literal `.env*` is gitignored (REVIEW-008 finding 3: the rules now use `.env*`, not `.env` + `.env.*`); `.env.example` itself is the sole negation and is tracked | PASS | `gates.txt`, ".env hygiene" section — probes `.env`, `.env.local`, `.env.staging`, `.envrc`, `.envfoo`; negative probe shows `.env.example` not ignored; tracked check; the file's variable lines captured verbatim |
