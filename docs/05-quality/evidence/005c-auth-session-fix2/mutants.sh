#!/usr/bin/env bash
#
# Mutation harness for 005c — auth and session v1, fix cycle 2 (Unit D, CTRL-005).
#
# WHY THIS EXISTS
#
...

{
  ...
  begin_mutant "unpersisted-rotation-dropped-silently" \
    ...
    "M24 / ADR-007 clause 3 — a refused session write is surfaced, not dropped"
  ...
  if baseline 'secure-store-adapter' 'holds a removal behind an in-flight reader, so the reader still sees the whole session'; then
    ...
    verdict 'secure-store-adapter' 'holds a removal behind an in-flight reader, so the reader still sees the whole session'
  fi

  begin_mutant "payload-parsed-behaviour-preservingly" \
    "the adapter parses the payload without changing what it returns" \
    "M28 / claim 12 — the adapter never parses token material"
  if baseline 'token-opacity' 'finds no token inspection anywhere in the adapter source'; then
    ...
    edit "$ADAPTER" \
      ...
      "$(printf "      const value = parts.join('');\n      try {\n        JSON.parse(value);\n      } catch {}")"
    verdict 'token-opacity' 'finds no token inspection anywhere in the adapter source'
  fi
  ...
  begin_mutant "ceiling-lowered-below-a-persistable-session" \
    ...
    "M29 / claim 11 — the ceiling admits every session auth-js can hand this adapter"
