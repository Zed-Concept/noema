#!/usr/bin/env bash
#
# Evidence producer for 005c — auth and session v1, fix cycle 2 (Unit D, CTRL-005).
#
# Writes every artifact in this directory except README.md, mutants.txt,
# stability.txt, and ci.txt. `ci.txt` is named explicitly because REVIEW-020
# finding 7 caught the cycle-1 manifest omitting it: a manifest that lists four
# exceptions and has five is a manifest nobody can check against the directory.
...
# The base this cycle is measured against is CURRENT MAIN, not the original
# dispatch base. Main was merged into this branch for fix cycle 1, so
# merge-base(main, HEAD) is main's own tip and this range is exactly what the
# unit contributes on top of it — including ADR-004, ADR-005, and ADR-006, which
# are main's records and are not attributed to this unit.
...
# -------------------------------------------------------------- dependencies
{
  ...
  echo "# Fix cycle 1 adds NO dependency: the checksum ADR-006 requires is"
  echo "# computed inline, with no crypto API and no new package."
  echo
