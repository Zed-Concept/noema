...
| 23 | The auth-settings preflight fails closed: any `/auth/v1/settings` response that is not HTTP 200 with a parseable body carrying boolean `disable_signup` and `mailer_autoconfirm` aborts the run at exit 4 **before any probe runs**, in both modes, and a well-formed response still lets the run continue. **Each half of the claim is proved by the artifact rather than by the source**: every case runs `--anon` *and* `--auth`, so the auth mode's own preflight call is exercised; four cases hold one flag valid while the other is invalid, so neither boolean guard can be removed without turning cases red; and "still accepts" is proved by the production run continuing into probes, not by a flag that stops it (REVIEW-016 finding 2) | PASS | `settings-preflight-control.txt` section 1 (10 responses x 2 production modes = 20 cases: 18 abort-at-4 with exact reason and zero probe `PASS`, 2 continue past the preflight into probes; all exactly as pinned) |
...
| 25 | **No control variable can green a production run.** No test hook exists in `rls-probes.mjs` or `live-probes.sh` at all — the retired `SETTINGS_PREFLIGHT_CONTROL` seam, which made `--anon` exit 0 after the preflight with zero probes and which the wrapper inherited and read as success, is gone — and `live-probes.sh` refuses to start, at exit 5 before anything is read, written, or contacted, if **any** control variable this suite defines is present in its inherited environment. It aborts rather than clearing the variable, in both of its modes, and a retired name stays on the refusal list so a stale ambient value still surfaces (REVIEW-016 finding 2) | PASS | `settings-preflight-control.txt` section 2 (3 refusals — the leak variable in both modes and the retired variable — each exit 5 with the refusal message and an untouched output directory, plus the clean-environment positive that completes at exit 0) |
