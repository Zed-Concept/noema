...
| 25 | **An ambient control variable cannot silently switch a production run, in the three variable/mode pairs measured.** The `SETTINGS_PREFLIGHT_CONTROL` settings-preflight success seam — which made `--anon` exit 0 after the preflight with zero probes and which the wrapper inherited and read as success — is **gone**. That is narrower than "no test hook at all", which would be false: `rls-probes.mjs` still carries the contained synthetic `REDACTION_CONTROL_LEAK` hook and `live-probes.sh` still has its `--control` mode (REVIEW-017 finding 4). `live-probes.sh` refuses to start at exit 5, aborting rather than clearing the variable, and a retired name stays on the refusal list so a stale ambient value still surfaces (REVIEW-016 finding 2). **The measured pairs are three, not all four**: `REDACTION_CONTROL_LEAK` in the default mode, `REDACTION_CONTROL_LEAK` under `--control`, and the retired `SETTINGS_PREFLIGHT_CONTROL` in the default mode. The retired name under `--control` is **not exercised by any artifact**, so no claim here quantifies over every control variable in both modes (REVIEW-017 finding 3); the guard's source enumerates both names for both modes, but source reading is not the evidence standard AGENTS.md sets. **The refusal precedes** every `.env`/credential read, every write, and any network contact — **not** the wrapper's own directory resolution and `git rev-parse --show-toplevel`, which run first (REVIEW-017 finding 4) | PASS for the three measured pairs and the clean-environment positive; the fourth pair is NOT RUN | `settings-preflight-control.txt` section 2 (3 refusals, each exit 5 with the refusal message and an untouched output directory, plus the clean-environment positive that completes at exit 0) |
