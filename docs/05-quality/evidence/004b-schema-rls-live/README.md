...
(regenerates byte-for-byte from its committed script — proven per artifact by
`stability.txt`), **run-varying** (varying fields named), and **not gated**
(the gate itself). **That definition is not satisfied at this head.** The
committed `stability.txt` is the fix-cycle-5 result, and no capture has been
run since the fix-cycle-6 producer edits; `settings-preflight-control.txt` is
known **stale** and will not reproduce here. Read every "gated" label in the
table below as *gated in principle, last proven at fix cycle 5* — the
per-artifact position is claims 15 and 26, which govern. Every producer pins
...
| `settings-preflight-control.txt` | `capture.sh` → `settings-control.mjs` | gated in principle; **STALE at this head — see claim 26**: these are the fix-cycle-5 bytes, and the fix-cycle-6 producer was not run because the dispatch prohibits invoking `live-probes.sh` in any form | the REVIEW-015 finding 2 and REVIEW-016 finding 2 control, in two sections. The **settings-preflight success hook** is gone from both producers; the contained synthetic redaction hook and `live-probes.sh --control` remain (REVIEW-017 finding 4). Every case drives a real production entry point. **Section 1's** cases run with no control variable set; **section 2 deliberately sets one** in three of its four cases — that is the variable under test there (REVIEW-018 finding 3, correcting an earlier blanket "every case" statement that section 2 contradicts). The same blanket wording survives in `settings-control.mjs`'s generated header and therefore in the committed transcript's own header lines; those bytes are the stale fix-cycle-5 capture and are not corrected here (claims 15 and 26). **Section 1 (20 cases):** ten `/auth/v1/settings` responses x both production modes, `--anon` **and** `--auth`, against a loopback HTTP server that controls that response and 404s every other route. Nine negative — unreachable (`HTTP 0`, the exact shape the committed `auth-probes.txt` recorded), HTTP 503, unparseable body, `{}`, null flags, a non-boolean `disable_signup` **with a valid `mailer_autoconfirm`**, and a missing / null / non-boolean `mailer_autoconfirm` **with a valid `disable_signup`** — each must abort at exit 4 with its exact recorded reason and emit zero probe `PASS` lines — the fix-cycle-6 producer requires zero probe lines of **any** classification, but that stricter form has not been run (claim 26). One positive — well-formed booleans — must **continue past the preflight into probes** in both modes (exit 1 anon, 3 auth against the 404 server), which is what makes the nine aborts mean something. Running every case in both modes exercises the auth mode's own preflight call, and the four isolating cases mean neither boolean guard can be deleted without turning cases red. **Section 2 (4 cases):** `live-probes.sh` handed an ambient control variable must refuse at exit 5 with its refusal message and leave the handed output directory **absent or empty when the run ends** (that is the whole of what the oracle checks: `existsSync(outdir)` is false, or else `readdirSync(outdir).length === 0` — a final-state check, not an ordering one, and it is weaker than the "untouched" the transcript's own annotation prints — REVIEW-018 finding 2, claim 25) — for `REDACTION_CONTROL_LEAK` in the default mode, for the retired `SETTINGS_PREFLIGHT_CONTROL` in the default mode, and for `REDACTION_CONTROL_LEAK` in `--control` mode — while a clean environment must still be ADMITTED and complete. That is **three of the four** variable/mode pairs; the retired name under `--control` is not run, so the matrix is not exhaustive over the guard's own variable list (REVIEW-017 finding 3). Synthetic key, `127.0.0.1` only, children that read their URL and key from the environment and never from `.env`: no Supabase project, credential, or network host is contacted. Deterministic — the ephemeral port is redaction-registered by the child, so it never reaches the output, and no timing or path is printed |
| `settings-control.mjs` | authored control harness | producer (not an output) | owns the loopback server, the 20-case both-mode preflight matrix, and the 4-case control-variable containment matrix behind `settings-preflight-control.txt`; run by `capture.sh`, never by a live run. It strips every control variable from the environment it hands its children, so it cannot itself become the ambient switch it exists to rule out. **Narrowed in fix cycle 6** (REVIEW-017 finding 2): a negative case is now accepted only on zero probe lines of any classification, where it previously accepted on zero probe `PASS` lines. The committed transcript predates that narrowing and was not regenerated — claim 26 |
...
| 11 | The four non-install CI steps pass at the measured head (typecheck, lint, test, format:check — all exit 0). Install is NOT RUN here: the delta provably contains no dependency change (probe in the transcript), and 002d/003a document the destructive npm-ci ENOTEMPTY transient under a live editor; CI runs the real install when the PR opens. **Measured at the fix-cycle-6 head, not re-run at this one** — the fix-cycle-7 dispatch authorizes no capture; that cycle changed only `docs/` prose, which is prettier-ignored, reaches neither `expo lint` nor `tsc` nor Jest, and moves no package file. | PASS at the fix-cycle-6 head / install NOT RUN with reason | `gates.txt` |
...
| 15 | The six gated artifacts regenerate byte-for-byte (two fresh capture runs, locale pinned) | **NOT RUN at this head** — the committed `stability.txt` is the fix-cycle-5 result and was true of the fix-cycle-5 producers. Fix cycle 6 edited `settings-control.mjs`, `live-probes.sh`, and this directory's `capture.sh`, and the only way to regenerate `settings-preflight-control.txt` and `redaction-control.txt` is to invoke `live-probes.sh`, which the fix-cycle-6 dispatch prohibits in any form. `settings-preflight-control.txt` therefore **will not** reproduce at this head — its section-1 negative cases will print a probe-line count instead of a probe-PASS count, and its trailers change (claim 26). `types-shape.txt`, `gates.txt`, `inventory.txt`, and `secret-scan.txt` are unaffected by those edits (`docs/` is prettier- and eslint-ignored, and no tracked non-docs file changed), but that is reasoning, not a fresh capture, and is not offered as a PASS | `stability.txt` (fix-cycle-5 result, superseded as a claim about this head) |
...
| 23 | The auth-settings preflight fails closed: any `/auth/v1/settings` response that is not HTTP 200 with a parseable body carrying boolean `disable_signup` and `mailer_autoconfirm` aborts the run at exit 4, in both modes, with its exact recorded reason and **zero probe `PASS` lines**, and a well-formed response still lets the run continue into probes. **The claim stops there.** The stronger boundary — *before any probe runs at all* — is **NOT established by the committed artifact**: the harness that produced it accepted a negative case on `probePassLines === 0`, so a probe that ran and FAILED before the abort would still have greened it (REVIEW-017 finding 2). `settings-control.mjs` has been narrowed in fix cycle 6 to require zero probe lines of **any** classification, which is what would establish the stronger boundary — but the narrowed harness has **NOT been run** (see claim 26), so nothing here claims its result. Of what remains, each half is proved by the artifact rather than by the source: every case runs `--anon` *and* `--auth`, so the auth mode's own preflight call is exercised; four cases hold one flag valid while the other is invalid, so neither boolean guard can be removed without turning cases red; and "still accepts" is proved by the production run continuing into probes, not by a flag that stops it (REVIEW-016 finding 2) | PASS at the narrowed boundary (exit 4 + exact reason + zero probe `PASS`); the "before any probe runs" boundary is NOT RUN | `settings-preflight-control.txt` section 1 (10 responses x 2 production modes = 20 cases: 18 abort-at-4 with exact reason and zero probe `PASS`, 2 continue past the preflight into probes; all exactly as pinned) |
| 24 | That the committed live transcripts were produced by the pre-fix producer | disclosed, not claimed — `auth-probes.txt` and `anon-probes.txt` were captured in fix cycle 3, before claim 23's preflight existed, and neither fix cycle 4 nor fix cycle 5 ran live probes by dispatch. A re-run under the current producer would have aborted at the auth transcript's recorded `HTTP 0`. The transcripts are retained as the fix-cycle-3 evidence boundary with that provenance stated; they are not re-labeled as if the preflight had gated them | — |
| 25 | **An ambient control variable makes the wrapper refuse to start, in the three variable/mode pairs measured — the refusal is loud (exit 5, refusal message) and the run produces no output.** The `SETTINGS_PREFLIGHT_CONTROL` settings-preflight success seam — which made `--anon` exit 0 after the preflight with zero probes and which the wrapper inherited and read as success — is **gone**. That is narrower than "no test hook at all", which would be false: `rls-probes.mjs` still carries the contained synthetic `REDACTION_CONTROL_LEAK` hook and `live-probes.sh` still has its `--control` mode (REVIEW-017 finding 4). `live-probes.sh` refuses to start at exit 5, aborting rather than clearing the variable, and a retired name stays on the refusal list so a stale ambient value still surfaces (REVIEW-016 finding 2). **The measured pairs are three, not all four**: `REDACTION_CONTROL_LEAK` in the default mode, `REDACTION_CONTROL_LEAK` under `--control`, and the retired `SETTINGS_PREFLIGHT_CONTROL` in the default mode. The retired name under `--control` is **not exercised by any artifact**, so no claim here quantifies over every control variable in both modes (REVIEW-017 finding 3); the guard's source enumerates both names for both modes, but source reading is not the evidence standard AGENTS.md sets. **What the artifact establishes about ordering — and what it does not (REVIEW-018 finding 2).** For each measured pair the oracle records exactly three things: the wrapper exited 5, the refusal message was printed, and the handed output directory was **absent or empty when the run ended**. That last check passes when `existsSync(outdir)` is false, or else when `readdirSync(outdir).length === 0` — a **final-state** test, not an ordering one. A run that created the directory before refusing, or created an entry and removed it, would still satisfy it. The oracle instruments **no** `.env`/credential read, **no** write outside the handed directory, and **no** network activity whatsoever. The stronger boundary — *the refusal precedes every credential/`.env` read, every write, and any network contact* — is therefore **NOT RUN** here. It is supported only by reading `live-probes.sh`, where the guard sits above those operations and below the wrapper's own directory resolution and `git rev-parse --show-toplevel`, which run first (REVIEW-017 finding 4); source reading is not the evidence standard AGENTS.md sets, and no artifact in this directory measures it. The committed transcript prints the stronger wording in its own per-case annotation ("the refusal precedes every read and write"): **that annotation outruns its own oracle**, is superseded by this claim, and was not rewritten because its bytes are the stale fix-cycle-5 capture (claims 15 and 26) | PASS for the three measured pairs — bounded to exit 5, the refusal message, and an absent-or-empty handed output directory — plus the clean-environment positive; the read/write/network precedence boundary is **NOT RUN** (uninstrumented; source-inspected only); the fourth variable/mode pair is **NOT RUN** | `settings-preflight-control.txt` section 2 (3 refusals, each exit 5 with the refusal message and an absent-or-empty handed output directory, plus the clean-environment positive that completes at exit 0 and writes `redaction-control.txt`) |
| 26 | **`settings-preflight-control.txt` was not regenerated in fix cycle 6 and no longer matches its producer.** The committed transcript is the fix-cycle-5 capture. Fix cycle 6 narrowed `settings-control.mjs`'s negative-case acceptance from "zero probe `PASS` lines" to "zero probe lines of any classification" (REVIEW-017 finding 2) and corrected producer prose (findings 3 and 4). Regenerating the transcript means running `settings-control.mjs`, which spawns `live-probes.sh`; the fix-cycle-6 dispatch prohibits invoking `live-probes.sh` **in any form**, so the run was not made. Consequences, stated rather than papered over: the committed artifact proves only what the **old** predicate checked (claim 23), the narrowed predicate has no artifact at all, and byte-stability is not proven at this head (claim 15). Nothing in this directory is upgraded on the strength of the source edit | disclosed, not claimed — the artifact is the fix-cycle-5 boundary and is labelled as such | `settings-preflight-control.txt` (fix-cycle-5 bytes, unchanged) |

## Producer source comments that outrun this record (REVIEW-018 finding 3)

Two producers carry comments that claim more than anything here measures.
**Both files are byte-untouched in this cycle, deliberately**, and **this
README is the claim boundary** — where a source comment and a claim above
disagree, the claim governs.

- **`rls-probes.mjs` line 290** reads `NO TEST HOOK LIVES BELOW THIS LINE,
  DELIBERATELY`. That is **false as written**: the synthetic
  `REDACTION_CONTROL_LEAK` hook is 40 lines below it, at line 330 inside
  `anonMode()`. The sentence was true of the *settings-preflight success*
  hook it was written about (REVIEW-016 finding 2), and it was never narrowed
  when the redaction hook stayed. What is actually true is claim 25's
  narrower form: the settings-preflight success seam is gone, while the
  redaction hook remains and is **contained** — it fires only when
  `REDACTION_CONTROL_LEAK=1` *and* the key carries the `SYNTHETIC` marker, so
  it cannot emit a real credential, and `live-probes.sh` refuses to start at
  all when the variable is ambient. `rls-probes.mjs` is the producer of the
  three protected live transcripts (`anon-probes.txt`, `auth-probes.txt`,
  `redaction-gate.txt`), whose committed bytes are bound to their GREEN
  SHA-256s; editing it in a cycle that cannot re-run it would desynchronize
  those protected artifacts from their producer, so the comment is corrected
  **here** rather than there.
- **`settings-control.mjs`'s generated header** states that every case runs
  with no control variable set. Section 2 sets one in three of its four
  cases. Those header lines are printed into
  `settings-preflight-control.txt`, whose committed bytes are the stale
  fix-cycle-5 capture (claims 15 and 26); the transcript's per-case
  annotation likewise prints an ordering claim its own oracle does not check
  (claim 25). Neither can be corrected without a run this dispatch prohibits,
  and neither is corrected by hand — an evidence transcript is measured
  output, not editable prose.

`../004a-schema-rls/README.md` records the same kind of divergence for
`verify-migrations.mjs`, on the same reasoning.
...
- `bash docs/05-quality/evidence/004b-schema-rls-live/capture.sh` —
  ...
  CI steps, the redaction positive control, and the 24-case auth-settings
  preflight control — 20 preflight cases plus 4 wrapper-containment cases; a
  couple of minutes). Exit 1 = fail closed. **It invokes `live-probes.sh`**
  (for the redaction positive control, and again inside
  `settings-control.mjs`), which is why fix cycle 6 could not regenerate
  `settings-preflight-control.txt` — claims 15 and 26.
