#!/usr/bin/env bash
#
# Evidence producer for 006b — session durability fix cycle 1 (Unit E, CTRL-006).
#
# Inherited from ../006a-session-durability/capture.sh and re-based for this
# cycle with the REVIEW-023 finding 5 hardening:
#
#   EVERY GIT INVOCATION'S EXIT STATUS IS CHECKED, and any non-zero exit fails
#   the capture. The reviewer's negative control made every `git diff` return
#   77; the 006a producer still exited 0 and reported an empty range with all
#   its regex controls matched, because those controls test pattern
#   recognition, not producer success. The committed control for THIS producer
#   is capture-refusal-control.sh: the same PATH-prepended refusal must make
#   this script exit non-zero.
#
#   THE CAPTURE BINDS ITSELF: binding.txt records the exact candidate SHA
#   (`git rev-parse HEAD`) and a clean-tree verdict (`git status --porcelain`,
#   empty apart from this cycle's own evidence directory and the output
#   directory — the measurement record is excluded; the measured program is
#   not). A dirty tree beyond those exclusions fails the run before anything
#   else is written.
#
# Writes every .txt artifact in this directory except README.md, mutants.txt,
# stability.txt, finding3-probe.txt, review023-probe.txt,
# capture-refusal-control.txt, and ci.txt — SEVEN exceptions, counted here
...
# claim-48a ruling). finding3-probe.txt and review023-probe.txt are written by
# their own runners, which run jest in disposable worktrees and cannot live
# inside a same-tree capture. capture-refusal-control.txt is written by
...
# their 006a producers' contracts.
#
...
# exactly what Unit E, its review, and this fix cycle contribute on top of it.
#
...
# ------------------------------------------------------------------ binding
# REVIEW-023 finding 5: the artifacts did not bind themselves to a candidate
# SHA or a clean tree. They do now, and a run against a dirty tree refuses
...
{
  echo "# Binding — REVIEW-023 finding 5."
  echo "# The candidate this artifact set describes, recorded by the producer at"
  echo "# run time. Every git invocation in this capture checks its own exit"
  echo "# status; any non-zero exit fails the run (the committed negative control"
  echo "# is capture-refusal-control.sh / capture-refusal-control.txt)."
  echo
  echo "base:  $BASE"
  ...
  echo "note:  this file is byte-stable across runs AT ONE HEAD. A stability rerun"
  echo "       at a different head differs on the head line BY DESIGN — that is"
  echo "       the binding working; stability.sh masks exactly that line when"
  echo "       comparing against the committed copy and prints both values."
} > "$OUT/binding.txt"
...
# ------------------------------------------------- storage property transcript
{
  ...
  echo "# adapter itself (including the exact-address read-back set, REVIEW-023"
  echo "# finding 5), the platform split, the keychain accessibility class, and"
  echo "# the wiring into the Supabase client. Runs against in-memory doubles:"
  echo "# no device, no native module, no credentials."
  echo
...
# ------------------------------------------------ session/OTP property transcript
{
  ...
  echo "# REVIEW-023 fixes — the observed purge (read-back as the only proof),"
  echo "# signedOut BEFORE the purge await with the mid-purge event gate"
  echo "# (finding 2), the durable demand with ruling 25's memory hold and"
  echo "# retry (finding 1), and the record-then-absorb write path with no"
  echo "# remaining rethrow case."
  echo "#"
  echo "# THREE SUITES, split as in 006a: auth-provider proves the provider's"
  echo "# wiring over doubles; foreground-refresh runs the REAL adapter, REAL"
  echo "# observer, and REAL demand module over in-memory backends;"
  echo "# reauth-demand proves the demand module's own contract, the ruling-25"
  echo "# hold and retry included. No network call and no credential."
  echo
  session_status=0
...
# ----------------------------------------------------------------- RED lane
{
  ...
  # docs/ is applied to any database by any path in this repo.
  excluded_paths=$(git diff --name-only "$BASE" -- 'docs/'); excl_code=$?
  if git_exit "$excl_code" "git diff --name-only $BASE -- docs/"; then
    echo "  docs/ paths excluded from the added-line scan: $(printf '%s\n' "$excluded_paths" | grep -c . | tr -d ' ')"
...
# ------------------------------------------------------------------- chrome
{
  ...
  # The positive control 006a's version lacked: a synthetic file that DOES
  # contain the gated name, assembled from fragments so this script is not
  # itself a hit, must match through the same pipeline.
  NAME_CONTROL="$(mktemp)"
...
# -------------------------------------------------------------- dependencies
{
  ...
  echo "# This fix cycle adds NOTHING. REVIEW-023 finding 5: the 006a artifact"
  echo "# printed the direct dependency but did not prove the lockfile"
  echo "# package-key set stayed identical — this one does, from the lockfile"
  echo "# objects themselves."
  echo
