#!/usr/bin/env bash
#
# Evidence producer for 005d — auth and session v1, fix cycle 3 (Unit D, CTRL-005).
#
# Writes every artifact in this directory except README.md, mutants.txt,
# stability.txt, session-sizes.txt, and ci.txt. FIVE exceptions, counted here
# rather than asserted: the cycle-2 version of this line named four and the
# directory had five, omitting session-sizes.txt — the same defect REVIEW-020
# finding 7 caught for ci.txt one cycle earlier, reintroduced by the file that
# fixed it. A manifest nobody can check against the directory is not a manifest.
...
# The base this cycle is measured against is CURRENT MAIN — the PR #14 merge
# commit, merged into this branch at b5c9cee, so this range is exactly what the
# unit contributes on top of it. ADR-004 through ADR-008 are main's records and
# are not attributed to this unit.
#
# PINNED, AND THEN CHECKED. REVIEW-021 finding 6 caught the cycle-2 value still
# holding cycle 1's main: main had moved twice underneath it, so the diff this
# producer measured silently included commits that were main's, and a rerun at a
# later head did not reproduce the committed artifacts. A literal is still the
# right shape — deriving the base from `git merge-base main HEAD` would read a
# LOCAL ref that can be arbitrarily stale, which is exactly how this cycle's
# preflight first saw ADR-008 as missing from main when it was present on
# origin/main. So the literal stays and the producer verifies it, failing closed
# rather than measuring against a base that is no longer the right one.
BASE=6c925d1c5b5e9aa4f8da660028482707e3763c8a
...
# ------------------------------------------------ session/OTP property transcript
{
  ...
  echo "# device-local sign-out, and ADR-007's refresh lifecycle — the foreground"
  echo "# gate, and a rotated session that could not be persisted forcing"
  ...
  session_status=0
  for suite in auth-provider foreground-refresh; do
    npm test -- --ci --runInBand --verbose --testPathPattern "$suite" 2>&1 | mask
    code=${PIPESTATUS[0]}
...
# -------------------------------------------------------------- dependencies
{
  echo "# Dependency delta against the base."
  echo "#"
  echo "# SCOPE: this is the WHOLE UNIT delta against main, not this fix cycle."
  echo "# expo-secure-store therefore appears below as added, because the unit"
  echo "# added it in Phase A, before any fix cycle existed. REVIEW-021 finding 7"
  echo "# caught the previous header calling this a fix-cycle figure while the"
  echo "# README claimed the cycle added nothing: two true statements about two"
  echo "# different ranges, printed as though they were one. They are separated"
  echo "# now, and the per-cycle statement lives in the README beside the claim"
  echo "# that makes it, where its range can be read off the claim itself."
  echo "#"
  echo "# Authorized for this unit: expo-secure-store, and nothing else."
  echo "# Fix cycles 1, 2 and 3 add NO dependency between them: the checksum"
  echo "# ADR-006 requires is computed inline, with no crypto API and no new"
  echo "# package, and the token-opacity scan uses the TypeScript compiler that"
  echo "# is already a devDependency."
  echo
