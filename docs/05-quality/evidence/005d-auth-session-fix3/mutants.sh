#!/usr/bin/env bash
#
# Mutation harness for 005d — auth and session v1, fix cycle 3 (Unit D, CTRL-005).
#
# WHY THIS EXISTS
#
...
#
# Each mutant is checked THREE ways, which is what makes a RED result mean
# something:
#
...
#   build     the MUTATED TREE MUST TYPECHECK. New in this cycle, and the reason
#             is REVIEW-020 finding 5, promoted to learning 16: "a mutation that
#             fails typecheck is not a counterfactual." Cycle 1's M4 edit left
#             TypeScript unable to narrow the read union and `npm run typecheck`
#             exited 2, yet Jest's Babel path executed it anyway and the harness
#             scored it SENSITIVE. A mutant that does not compile is not a
#             version of this program that could exist, so it cannot be evidence
#             about this program. It is now recorded BUILD-INVALID and fails the
#             run rather than counting.
...
#
# WHAT A SENSITIVE COUNT IS AND IS NOT
#
# `n/n SENSITIVE` is an EXECUTION FACT: each named edit landed, the tree still
# compiled, and the named instrument turned red. It is NOT a semantic claim that
# the battery covers every behaviour, and it is not a coverage measure. Learning
# 16 states the distinction and REVIEW-020 finding 5 is what earned it.
#
# SAFETY
#
...
# Does the MUTATED tree compile? Learning 16: a mutant that does not typecheck
# is not a counterfactual, because it is not a program this project could ship.
# Jest's Babel path strips types and runs it regardless, which is exactly how a
# non-compiling mutant came to be scored SENSITIVE in cycle 1.
typechecks() {
...
verdict() {
  ...
  if typechecks; then
    ...
    echo "           a counterfactual: Jest's Babel path would execute it anyway and"
    echo "           score it red for the wrong reason. This is what REVIEW-020"
    echo "           finding 5 caught in cycle 1's M4."
...
FOREGROUND=src/lib/auth/foreground-refresh.ts
...

{
  ...
  # ------------------------------------------------------------ INVARIANT 1
  # REWRITTEN THIS CYCLE — REVIEW-020 finding 5. The cycle-1 edit was
  # `if (!indexRead.ok) {` -> `if (!indexRead.ok && false) {`, and it was
  # defective twice over:
  #
  #   1. NOT BUILD-VALID. `&& false` destroys TypeScript's narrowing of the
  #      `BackendRead` union, so `indexRead.value` below it raises two TS2339
  #      diagnostics and `npm run typecheck` exits 2. Jest's Babel path ran it
  #      anyway. The build gate above now refuses to score such a mutant, and
  #      this edit is build-valid: it returns the other member of the same
  #      union, so narrowing is untouched.
  #   2. NOT LOAD-BEARING. Its named test refused the FIRST replacement chunk
  #      write, so the laundering mutant changed no stored byte: the old session
  #      survived, the safety postcondition still held, and the test went red
  #      only because the refusal-specific error text did not match. That is an
  #      attribution false red. The test now allows one chunk write to land
  #      first, and asserts the no-write postcondition BEFORE the error's
  #      identity, so this mutant is red because it overwrote a live chunk of
  #      generation 0 in place.
  #
  # The edit itself is the cleaner statement of the defect anyway: invariant 1
  # is "absence is not failure", and this collapses exactly that distinction at
  # the point it is made.
  begin_mutant "index-read-refusal-laundered" \
    "a refused backend read reports itself as an absent key, so setItem picks a generation it cannot see" \
  ...
  if baseline 'secure-store-adapter' 'reports failure when only the index delete is refused'; then
    # The explicit `: boolean` is load-bearing FOR THE MUTANT, not for the
    # product. Without it TypeScript infers the literal type `true` from
    # `(...) || true`, and the `complete = false` assignments further down then
    # raise TS2322 — a BUILD-INVALID mutant, which the build gate above rejects.
    # Cycle 1 shipped it without the annotation and scored it SENSITIVE anyway.
    edit "$ADAPTER" \
      '    let complete = await deleteBackend(key);' \
      '    let complete: boolean = (await deleteBackend(key)) || true;'
  ...
  if baseline 'secure-store-adapter' 'writes an index of its own metadata only, with no field off the payload'; then
    # Leaked through the SERIALIZED object rather than by adding a field to the
    # `ChunkIndex` literal. `ChunkIndex` is a closed type, so the cycle-1 edit
    # tripped TypeScript's excess-property check and was BUILD-INVALID — it was
    # nonetheless scored SENSITIVE. Spreading into a fresh inferred object leaks
    # exactly the same payload bytes into exactly the same stored value, and
    # compiles.
    edit "$ADAPTER" \
      '    await backend.setItemAsync(key, JSON.stringify(index));' \
      '    await backend.setItemAsync(key, JSON.stringify({ ...index, head: value.slice(0, 32) }));'
  ...
  # ---------------------------------------------------------- ADR-007 / C
  # The two cycle-1 mutants that lived here — "appstate-gate-always-starts" and
  # "appstate-gate-ignores-mount-state" — are DELETED, not repaired. They
  # mutated `startAutoRefresh`/`stopAutoRefresh` calls, and REVIEW-020 finding 1
  # proved those calls never established the property they were credited with.
  # ADR-007 removed the scheduler instead of patching it, so there is no ticker
  # left to mutate. What replaces them are mutants against the three things
  # ADR-007 actually decides.
  begin_mutant "client-self-schedules-again" \
    "the auth client is constructed so it schedules its own refreshes" \
    "M22 / ADR-007 clause 1 — autoRefreshToken is false at construction"
  if baseline 'supabase-client' 'never lets the client self-schedule a refresh'; then
    edit "$CLIENT" '    autoRefreshToken: false,' '    autoRefreshToken: true,'
    verdict 'supabase-client' 'never lets the client self-schedule a refresh'
  fi

  begin_mutant "foreground-gate-removed" \
    "the gate stops testing AppState, so a refresh is initiated while backgrounded" \
    "M23 / ADR-007 clause 2 — refresh is initiated only by foreground-gated calls"
  if baseline 'foreground-refresh' 'initiates no refresh in AppState \"background\"'; then
    # `'unknown'` rather than deleting the line: it is a real member of
    # AppStateStatus, so the mutated tree still typechecks (learning 16) while
    # the gate no longer holds for background or inactive.
    edit "$FOREGROUND" \
      "  if (status !== 'active') return 'not-foreground';" \
      "  if (status === 'unknown') return 'not-foreground';"
    verdict 'foreground-refresh' 'initiates no refresh in AppState \"background\"'
  fi

  begin_mutant "unpersisted-rotation-dropped-silently" \
    "a rotated session that could not be written is reported as settled" \
    "M24 / ADR-007 cl.3 + ADR-008 — on NATIVE a refused write is surfaced, not dropped"
  if baseline 'foreground-refresh' 'reports \"unpersisted\" when the store refuses the rotated write'; then
    edit "$FOREGROUND" \
      "  return deps.takePersistenceFailure() ? 'unpersisted' : 'settled';" \
      "  return 'settled';"
    verdict 'foreground-refresh' 'reports \"unpersisted\" when the store refuses the rotated write'
  fi

  begin_mutant "write-observer-records-nothing" \
    "the session-write observer stops recording a refused write" \
    "M25 / ADR-007 clause 3 — the refused write is what is observed, not the rejection"
  if baseline 'foreground-refresh' 'observes without absorbing: the caller still sees the rejection'; then
    edit "$STORAGE" \
      '        lastPersistenceFailure = { key, cause };' \
      '        lastPersistenceFailure = null;'
    verdict 'foreground-refresh' 'observes without absorbing: the caller still sees the rejection'
  fi

  begin_mutant "reauthentication-not-forced" \
    "the provider keeps the session it could not vouch for" \
    "M26 / ADR-007 clause 3 — an unpersisted rotation forces re-authentication"
  if baseline 'auth-provider' 'requires re-authentication when a rotated session could not be persisted'; then
    edit "$PROVIDER" \
      "      if (active) setState({ status: 'signedOut' });" \
      '      void 0;'
    verdict 'auth-provider' 'requires re-authentication when a rotated session could not be persisted'
  fi

  # ------------------------------------------- REVIEW-020 findings 2, 3, 4
  begin_mutant "removal-bypasses-only-its-own-queue" \
    "removeItem stops participating in the serialization queue, while get and set still do" \
    "M27 / claim 13d — the queue covers EVERY public operation, removal included"
  if baseline 'secure-store-adapter' 'serializes a removal against a concurrent read, and the read returns the whole session'; then
    # REVIEW-020 finding 4's exact counterexample. It passed all 48 cycle-1
    # adapter tests: M1-M3 remove the queue globally, and none of them made
    # removal's OWN participation load-bearing.
    edit "$ADAPTER" \
      '    removeItem: (key) => serialized(() => removeItemBody(key)),' \
      '    removeItem: (key) => removeItemBody(key),'
    verdict 'secure-store-adapter' 'serializes a removal against a concurrent read, and the read returns the whole session'
  fi

  begin_mutant "payload-parsed-behaviour-preservingly" \
    "the adapter parses the payload without changing what it returns" \
    "M28 / claim 12 — no DIRECTLY-SPELLED parse of token material in the adapter"
  if baseline 'token-opacity' 'finds no directly-spelled token inspection in the adapter source'; then
    # REVIEW-020 finding 3's exact disposable violation. It is invisible to
    # every behavioural test BY CONSTRUCTION — it preserves behaviour — which is
    # why the instrument for this claim reads the source instead of the result.
    edit "$ADAPTER" \
      "$(printf "      const value = parts.join('');")" \
      "$(printf "      const value = parts.join('');\n      try {\n        JSON.parse(value);\n      } catch {}")"
    verdict 'token-opacity' 'finds no directly-spelled token inspection in the adapter source'
  fi

  begin_mutant "ceiling-lowered-below-a-persistable-session" \
    "the chunk ceiling returns to 64, refusing a session the pinned client can persist" \
    "M29 / claim 11 — the ceiling admits the largest session shape measured here"
  if baseline 'secure-store-adapter' "admits REVIEW-020 finding 2's 100,000-character-metadata session"; then
    edit "$ADAPTER" 'export const MAX_CHUNKS = 256;' 'export const MAX_CHUNKS = 64;'
    verdict 'secure-store-adapter' "admits REVIEW-020 finding 2's 100,000-character-metadata session"
  fi
  ...
  # ---------------------------------- REVIEW-021 finding 1 / advisory finding 1
  # The ungated entrances. Both reviewers converged here: removing the
  # self-scheduler closed one door and RELOCATED the other. The app's own
  # `onAuthStateChange` registration re-enters the margin refresh through
  # `_emitInitialSession` -> `_useSession` -> `__loadSession`, which consults
  # neither `autoRefreshToken` nor any foreground gate; bootstrap `getSession()`
  # was a second entrance into the same function. Both now sit behind the gate.
  #
  # ONE mutant, TWO claims. The deferral is a single decision in a single line,
  # so a single edit is the honest counterfactual for it. The listener claim and
  # the cold-start-read claim are both bounded by this mutant, and the claims
  # table says so rather than implying each has its own — the same convention
  # claim 6 already follows behind claim 5's mutant.
  begin_mutant "entrances-not-deferred" \
    "the cold-start read and the listener registration run at mount, ungated" \
    "M30 / claims 51+52 — neither entrance opens before the first foreground"
  if baseline 'auth-provider' 'initiates nothing at all when mounted while the app is backgrounded'; then
    edit "$PROVIDER" \
      "        if (status === 'active' && !bootstrapStarted) startBootstrap();" \
      "        if (!bootstrapStarted) startBootstrap();"
    verdict 'auth-provider' 'initiates nothing at all when mounted while the app is backgrounded'
  fi

  # ------------------------------------------------- REVIEW-021 finding 2
  # Durable re-authentication. Detection was already sound — the advisory
  # confirmed it sits at the write rather than the initiator — so these three
  # mutate what happens AFTER detection, which is where the gap was.
  begin_mutant "refused-write-erased-by-a-later-success" \
    "a later successful write clears an outstanding refusal before it is consumed" \
    "M31 / claim 53 — the refusal is sticky until the session layer takes it"
  if baseline 'foreground-refresh' 'keeps a refused write outstanding when a later write succeeds'; then
    edit "$STORAGE" \
      "$(printf "      // and the sign-in that follows is unaffected.\n    },")" \
      "$(printf "      // and the sign-in that follows is unaffected.\n      lastPersistenceFailure = null;\n    },")"
    verdict 'foreground-refresh' 'keeps a refused write outstanding when a later write succeeds'
  fi

  begin_mutant "refused-removal-not-recorded" \
    "the purge observer stops recording a removal the store refused" \
    "M32 / claim 54 — a refused removal is observed as its own fact"
  if baseline 'foreground-refresh' 'records a refused removal, and rethrows rather than absorbing it'; then
    edit "$STORAGE" \
      '        lastPurgeFailure = { key, cause };' \
      '        lastPurgeFailure = null;'
    verdict 'foreground-refresh' 'records a refused removal, and rethrows rather than absorbing it'
  fi

  begin_mutant "reauthentication-does-not-outlive-its-attempt" \
    "the removal is attempted once and forgotten, so a refusal leaves the superseded session readable" \
    "M33 / claim 55 — the demand survives a refusal and is retried until accepted"
  if baseline 'auth-provider' 'retries the removal on the next foreground when the store refused it'; then
    # Newline-anchored: the ten-space retry line at the bottom of `evaluate`
    # contains this six-space text as a substring, and `edit` refuses an anchor
    # that matches twice. The mutation keeps the FIRST attempt and drops only
    # its durability, which is exactly the defect finding 2 described.
    edit "$PROVIDER" \
      "$(printf "\n      purgeOutstanding = !(await purgeStoredSession());")" \
      "$(printf "\n      await purgeStoredSession();")"
    verdict 'auth-provider' 'retries the removal on the next foreground when the store refused it'
  fi
