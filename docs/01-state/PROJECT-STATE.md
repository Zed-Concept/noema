...
|---|---|---|---|
| Unit C — Schema and RLS v1 (`feat/schema-rls-v1`) | Claude Code | REVIEW-015 FAIL → **fix cycle 4 complete**, both verdict-driving findings closed **offline — no live run this cycle by controller ruling**. **F1 (in-class false greens):** reproduced all six reported neighbors on the committed oracle first (78/78, exit 0 each), then audited every one of the twelve named classes for the same defect shape and found **35** false greens, not six. Closed at two structural causes rather than 35 patches: a shared `opExpr()` helper that compares the A_Expr **kind** as well as the operator name — libpg_query names `IS DISTINCT FROM`, `IS NOT DISTINCT FROM`, `= ANY` and `= ALL` all `=`, and the first inverts the predicate, which is exactly why a storage folder-owner equality could be reversed while the oracle printed its `{user_id}/-scoped` PASS — and exact-key-set pinning for every node whose shape a class claims to pin, so an unaccounted-for grammar clause rejects instead of riding along unread. Oracle 78 → **91 assertions, 91 PASS** on unchanged migrations; **0 of 35 neighbors still green**. **The claim is now derived from the battery instead of asserted beside it:** every scenario carries its enumerated class, `capture.sh` computes the demonstrated set, parses the README's claimed list, and exits 1 unless they are identical in both directions — a class that cannot be demonstrated is removed from the claim, not defended in prose. Battery 32 → **55 permanent scenarios, 55/55 discriminating, 12/12 classes**. The gate proved itself in-cycle by failing closed on two of the builder's own malformed scenarios. **F2 (fail-open settings preflight):** `/auth/v1/settings` now fails closed at a distinct **exit 4** in both modes **before any probe runs** on any non-200, unparseable, or non-boolean response, with a new six-case permanent control (five negative — including the exact `HTTP 0` shape the committed transcript recorded — plus a positive control proving it still accepts a well-formed response). The claim was **narrowed onto the evidence that carries it**: the authenticated transcript's settings line is disclosed as recorded under the defective check and is no longer cited; what establishes `mailer_autoconfirm` was true at run time is that **46 authenticated probes obtained real sessions**, which the signup path cannot produce while confirmation is required. Committed transcripts deliberately **not** regenerated. REVIEW-015 finding 3 (low) and REVIEW-013 finding 4 remain controller-owned and untouched per the dispatch. Ran wholly as Opus 5 [1m] / Max, the owner-ruled temporary substitution for Fable 5 | Re-review by the reviewer of record + advisory (DeepSeek V4 Pro), then owner merge |
