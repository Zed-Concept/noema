...
|---|---|---|---|
| Unit C — Schema and RLS v1 (`feat/schema-rls-v1`) | Claude Code | REVIEW-012 FAIL → fix cycle 2 complete: every privileged-role/tooling/PUBLIC/service_role/column-ACL/definer-owner claim narrowed to the measured `roles-acl.txt` boundary (three new NOT RUN claims where the grid is silent), the schema oracle now pins absence as well as presence (78 assertions; 18 neighbor mutations red; negative control 8 → 12 scenarios), delete-on-red prose matched to the producer, live-response oracles tightened to one exact status + code with the anon subset relabeled 9 of 11, and one fresh live run under those oracles in a new `ctrl004d-*` namespace (anon 11, auth 46, both gates GREEN, owner-deleted after). Disposable-user claims are per-namespace from now on. Mid-cycle model transition to Opus 5 [1m], owner-ruled in-loop and recorded in the LOCK | Re-review by the reviewer of record + advisory (DeepSeek V4 Pro), then owner merge |
