...
|---|---|---|---|
| Unit C — Schema and RLS v1 (`feat/schema-rls-v1`) | Claude Code | REVIEW-017 FAIL → **fix cycle 6 complete**, run **subtraction-only** under the controller's binding stop rule and with **no live run of any kind** — no signup, no namespace, no toggle, and no invocation of `live-probes.sh` in any form. **F1 (the surviving in-class false green):** the stop rule fired, so the `Functions` class was **removed from the claimed list** rather than the oracle extended a fourth time. `verify-migrations.mjs` is byte-untouched and the baseline is unchanged at **91 assertions, 91 PASS**; the eight `Functions`-tagged scenarios went with the class, because the derivation cross-check requires every scenario to carry a tag naming a claimed class. Battery **80 → 72** permanent scenarios, **12 → 11** classes, matched both ways. The README now states plainly that **function-definition structure is not a pinned class**, names the cause (the oracle joins `opts.as.List.items` and never pins that list to one item, so a two-`AS`-item neighbor returns 91/91), and rebases the committed function definitions onto **direct inspection and REVIEW-014's source-verified analysis**. The oracle's old contract sentence — a neighbor is rejected *exactly when it changes a property some assertion names* — was withdrawn as the same finding's counterexample. **F2 (the control greened after FAIL probes):** `settings-control.mjs`'s negative-case predicate is strictly narrowed from `probePassLines === 0` to `probeLines === 0` — zero probe lines of any classification. **It has no artifact:** regenerating `settings-preflight-control.txt` means running the harness, which spawns `live-probes.sh`, which this dispatch forbids. So claim 23 is **rewritten down** to what the committed transcript proves (exit 4, both modes, exact reason, zero probe `PASS`) and the *before any probe runs* boundary is **NOT RUN**. **F3 (the incomplete containment matrix):** **narrowed, not completed** — completing the fourth variable/mode pair would have produced a case no artifact could record, which is the same AGENTS.md breach in a new place. Claim 25 now quantifies over the **three measured pairs** and names `SETTINGS_PREFLIGHT_CONTROL` under `--control` as unexercised. **F4 (five prose overstatements):** corrected to measurement — *no test hook at all* is false (the synthetic redaction hook and `--control` remain; only the settings-preflight success hook is gone); *before anything is read* is false (directory resolution and `git rev-parse` precede the guard); the guard mutant yields **3** violations, not 4, so the corrected reading is 9, 6, 6 and **3** (the 4 came from a `set -u` broken-script mutant, and the 3 is REVIEW-017's measurement adopted, not a fresh one); the rerun text says **24 cases**, not six; and the all-twelve-class audit claim is **deleted** rather than backfilled, since no artifact records it. **Disclosed, not hidden:** `settings-preflight-control.txt` no longer matches its producer, and 004b byte-stability is **NOT RUN** at this head — both stated outright in 004b claims 15 and 26. Fix-cycle-6 delta 9 files, +171/-183 excluding the self-counting HANDOFF insertion (10 files with it); protected paths empty; both live-transcript SHA-256s still equal their GREEN bindings. Ran wholly as Opus 5 [1m] / Max (owner-ruled substitution for Fable 5); **no workflow and no subagent were used**. | Re-review by the reviewer of record + advisory (DeepSeek V4 Pro), then owner merge — the controller owns the one place where the dispatch's 004b touch-set and its no-`live-probes.sh` exclusion cannot both be satisfied |
