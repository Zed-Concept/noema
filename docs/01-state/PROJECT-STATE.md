...
|---|---|---|---|
| Unit E — Session durability (REVIEW-022 finding 3) | Claude Code | **REVIEW-025 FAIL** (2026-08-26, Codex Sol / Ultra; record `214a4508`): exposure class recurs a third time — the publish-time barrier cannot retract queued or standing state, `signOut()` under a refused refresh leaves `signedIn`; lint enforcement bypassable by aliasing; docs-only evidence invariant false on review-record commits. REVIEW-024 finding 1 CLOSED, ruling-26 deletion HONOURED. **Stop rule fired; ruling 28 governs: fix cycle 3 of 3 is subtraction only**, dispatched at Fable 5 / Max on branch tip `f72e63faee263f81097f532447a93248c53d750f`. Ships with two HIGH Known Issues on PASS. Evidence `006d-session-durability-fix3/`. | REVIEW-026 |
