...
|---|---|---|---|
| Unit C — Schema and RLS v1 (`feat/schema-rls-v1`) | Claude Code | REVIEW-013 FAIL → fix cycle 3 complete, all three verdict-driving findings closed: the residual anon claim narrowed everywhere to the measured grid (no current table-level SELECT/INSERT/UPDATE/DELETE, with the four non-CRUD raw-ACL entries and the NOT RUN column ACLs named alongside) — which required regenerating the transcripts under the corrected producer, so one fresh live run in a new `ctrl004e-*` namespace (anon 11, auth 46, both gates GREEN, two users owner-deleted after, toggle state measured before and after rather than assumed); the six REVIEW-013 neighbor classes reproduced false-green then closed at their AST cause (78 assertions unchanged, 78 PASS) with the oracle claim rewritten as a bounded enumerated-assertion claim naming twelve pinned classes and stating plainly that it is not a proof of exhaustive schema equivalence; and the unsupported 18-neighbor claim replaced by the committed full battery — negative control 12 → **32 permanent scenarios**, 32/32 discriminating, the count derived from the run counter and cross-checked against the artifact's own enumeration, so no claim rests on a scratch-only run. REVIEW-013 finding 4 (stale Phase B LOCK denial label) deliberately untouched — controller-owned, to be superseded at close-out. Ran wholly as Opus 5 [1m] / Max, the owner-ruled temporary substitution for Fable 5 | Re-review by the reviewer of record + advisory (DeepSeek V4 Pro), then owner merge |
