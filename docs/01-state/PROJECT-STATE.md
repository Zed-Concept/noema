...
|---|---|---|---|
| Unit E — Session durability (REVIEW-022 finding 3) | Claude Code | **REVIEW-024 FAIL** (2026-08-26, Codex Sol / Ultra; record `055ac265`): consult gate still reads a thrown read plus `exists === false` as absence (HIGH); the exposure invariant recurs in class — `getSession()` publishes after its await with no demand boundary (HIGH, stop rule invoked); 006b bound to `74024465`, not the candidate, `red-lane.txt` not stable at the candidate (MEDIUM). REVIEW-023 findings 1, 3, 4, 6 closed or honoured; 2 open in class; 5 open. REVIEW-023-ADVISORY (`0de2e406`) adjudicated: leads 1–3 into the cycle-1 addendum, 4–6 duplicate or agree. **Fix cycle 2 of 3** dispatched at Fable 5 / Max on branch tip `d38b2ba4` (LOCK back to BUILD). Cycle 3, if needed, remedies the exposure class by subtraction. Draft PR #17 carries CI only. Evidence `006c-session-durability-fix2/`. | REVIEW-025 |
